  ...
  return Fliplet.DataSources.get({
    ...
    .catch(function (error) {
      ...
      if (!(error instanceof Error)) {
        Raven.captureMessage('Error loading data sources', {
          extra: {
            error: error
          }
        });
...
function fetchCurrentDataSourceEntries(entries) {
  return Fliplet.DataSources.connect(currentDataSourceId).then(function (source) {
    ...
    return Fliplet.DataSources.getById(currentDataSourceId, {
      ...
      currentDataSourceUpdatedAt = TD(new Date(), {
        format: 'lll',
        locale: locale
      });
    ...
    .catch(function onFetchError(error) {
      ...
      if (error instanceof Error) {
        ...
        Raven.captureMessage('Error accessing data source', {
          extra: {
            dataSourceId: currentDataSourceId,
            error: error
          }
  ...
  Fliplet.API.request('v1/data-sources/' + currentDataSourceId + '/versions')
    .then(function (result) {
      ...
      var versions = currentDataSourceVersions.map(function (version) {
        version.createdAt = TD(version.createdAt, {
          format: 'lll',
          locale: locale
        });
...
$('#app')
  ...
  .on('click', '[data-version-preview]', function (e) {
    ...
    var version = _.find(currentDataSourceVersions, {
      id: id
    });
